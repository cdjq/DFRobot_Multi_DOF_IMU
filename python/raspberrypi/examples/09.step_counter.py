# -*- coding: utf-8 -*
'''!
@file  09.step_counter.py
@brief  Step counter interrupt example
@details  This example demonstrates how to configure and use step counter interrupt.
@n  The example configures the sensor to generate an interrupt each time a step is detected.
@n  When walking, the sensor automatically counts steps and triggers an interrupt for each step,
@n  then reads and displays the accumulated step count.
...
'''!
  @brief Interrupt flag and step count
'''
step_detected = False
last_step_count = 0
...
def setup():
  print("\nStep Counter Interrupt Example (%s mode)\n" % mode)
  ...
  '''!
      @brief Configure INT1 step counter interrupt
  '''
  print("[4] Configuring INT1 step counter interrupt... ", end="")
  ...
  print("Tip: Start walking to trigger step counting\n")


def loop():
  '''!
  @brief Main loop - count steps
  '''
  global step_detected, last_step_count
