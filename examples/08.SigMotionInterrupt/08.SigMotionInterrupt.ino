/*!
 * @file 08.SigMotionInterrupt.ino
...
volatile bool sigMotionDetected = false;
uint32_t      sigMotionCount    = 0;
...
void setup()
{
  ...
  Serial.println("\nSignificant Motion Interrupt Example");
...
#error
  ...
  Serial.println("Tip: Move sensor significantly in the same direction to trigger interrupt");
  Serial.println("     (Significant motion requires more obvious directional movement than any motion)");
...
void loop()
{
  if (sigMotionDetected) {
    ...
    if (intStatus & INT1_2_INT_STATUS_SIG_MOTION) {
      sigMotionCount++;

      Serial.print("Significant motion detection #");
      Serial.print(sigMotionCount);
