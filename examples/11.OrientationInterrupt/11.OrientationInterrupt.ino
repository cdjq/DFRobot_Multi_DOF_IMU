/*!
 * @file 11.OrientationInterrupt.ino
...
volatile bool orientationDetected    = false;
uint32_t      orientationChangeCount = 0;

const char* getOrientationString(uint8_t orientation);
const char* getFaceString(uint8_t face);
...
void setup()
{
  ...
  Serial.println("\nOrientation Detection Interrupt Example");
  ...
  Serial.print("[4] Configuring INT1 orientation detection interrupt... ");
  while (!imu.setInt(DFRobot_Multi_DOF_IMU::eImuIntPin1, DFRobot_Multi_DOF_IMU::eInt1_2Orientation)) {
...
#error
  ...
  Serial.println("Tip: Rotate sensor to trigger orientation change interrupt");
  Serial.println("     (Supports 4 orientations: Portrait up/down, Landscape left/right)");
  Serial.println("     (Supports 2 face directions: Face up/down)");
...
{
  switch (orientation) {
    case ORIENT_TYPE_PORTRAIT_UP:
      return "Portrait Upright";
    case ORIENT_TYPE_LANDSCAPE_LEFT:
      return "Landscape Left";
    case ORIENT_TYPE_LANDSCAPE_RIGHT:
      return "Landscape Right";
    case ORIENT_TYPE_PORTRAIT_DOWN:
...
const char* getFaceString(uint8_t face)
{
  return (face == ORIENT_FACE_UP) ? "Face Up" : "Face Down";
}
