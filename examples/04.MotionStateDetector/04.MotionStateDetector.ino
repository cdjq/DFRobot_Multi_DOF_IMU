/*!
 * @file 04.MotionStateDetector.ino
...
void setup()
{
  ...
  Serial.println("\nMotion State Detection Example");
  ...
  Serial.print("[5] Configuring INT2 no motion interrupt... ");
  while (!imu.setInt(DFRobot_Multi_DOF_IMU::eImuIntPin2, DFRobot_Multi_DOF_IMU::eInt1_2NoMotion)) {
    Serial.println("Failed, please check pin and interrupt configuration!");
    delay(1000);
  }
  Serial.println("Success");
...
#error
  ...
  // Configure INT2 interrupt pin (no motion)
#if defined(ESP32)
  ...
  Serial.println("     Keep still to trigger no motion interrupt\n");
  delay(100);
}
