...
volatile bool noMotionDetected = false;
uint32_t      noMotionCount    = 0;
...
void setup()
{
  ...
  Serial.println("\nNo Motion (Still) Interrupt Example");
  ...
  Serial.print("[4] Configuring INT1 no motion interrupt... ");
  while (!imu.setInt(DFRobot_Multi_DOF_IMU::eImuIntPin1, DFRobot_Multi_DOF_IMU::eInt1_2NoMotion)) {
...
#error
  ...
  Serial.println("Tip: Keep sensor still to trigger interrupt");
