...
volatile bool flatDetected = false;
...
void setup()
{
  ...
  Serial.println("\nFlat Detection Interrupt Example");
  ...
  Serial.print("[4] Configuring INT1 flat detection interrupt... ");
  while (!imu.setInt(DFRobot_Multi_DOF_IMU::eImuIntPin1, DFRobot_Multi_DOF_IMU::eInt1_2Flat)) {
...
#error
  ...
  Serial.println("Tip: Place sensor horizontally on a flat surface to trigger interrupt");
  Serial.println("     (Device needs to remain stable for a period of time)");
